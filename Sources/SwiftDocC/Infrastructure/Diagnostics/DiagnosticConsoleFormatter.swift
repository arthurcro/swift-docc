...
extension DefaultDiagnosticConsoleFormatter {
    ...
    private func readSourceLines(_ url: URL) -> [String] {
        ...
        guard let content = contentsOfURL(of: url)
        else { return [] }
    ...
    private func contentsOfURL(of url: URL) -> String? {
        // TODO: Add support for also getting the source lines from the symbol graph files.
        guard let dataProvider = dataProvider
        else { return try? String(contentsOf: url) }

        let content = try? dataProvider.contentsOfURL(url)
        return content.flatMap { String(data: $0, encoding: .utf8) }
    }
